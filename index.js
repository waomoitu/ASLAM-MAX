...
const { createContext } = require("./Aslam/helper");
...
const evt = require("./Aslam/dullah");
    ...
    async restartWorker(cleanSession = false) {
        ...
        try {
            ...
            if (cleanSession || this.authErrorCount >= this.maxAuthErrors) {
                try {
                    const sessionDir = path.join(__dirname, "Aslam");
...
app.get('/health', (req, res) => {
    res.json({
        status: aslam ? 'connected' : 'disconnected',
...
// ENHANCED AUTHENTICATION FUNCTION WITH BETTER ERROR HANDLING AND FLEXIBLE SESSION NAMES
    ...
    try {
        ...
        if (!fs.existsSync(credsPath) || conf.session !== "zokk") {
            ...
            if (/^[a-zA-Z0-9]+_[a-z0-9]{5}$/i.test(conf.session)) {
                const sessionUrl = `https://aslam-seseion-store.pages.dev/${conf.session}.json`;
            ...
            // Handle direct Aslam max;;; session string
            else if (conf.session.startsWith('BWM-XMD;;;')) {
                ...
                throw new Error("Invalid session format - must be [name]_xxxxx or Aslam max;;; format");
            }
        }
    } catch (error) {
...
async function main() {
    try {
        ...
        const { state, saveCreds } = await useMultiFileAuthState(__dirname + "/Aslam");
        ...
        const sockOptions = {
            ...
            browser: ['Aslam max', "safari", "1.0.0"],
        ...
        aslam = makeWASocket(sockOptions);
        ...
        //============================================================================//
         ...
         let ibraah = { chats: {} };
const botJid = `${aslam.user?.id.split(':')[0]}@s.whatsapp.net`;
const botOwnerJid = `${aslam.user?.id.split(':')[0]}@s.whatsapp.net`; // Fixed: Changed from aslam.user to config
...
const handleDeletedMessage = async (deletedMsg, key, deleter) => {
    const context = createContext(deleter, {
        ...
        thumbnail: "https://url.bwmxmd.online/Dullah.fwzxhzl7.jpg"
    });
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message.conversation || deletedMsg.message.extendedTextMessage?.text) {
                        ...
                        await aslam.sendMessage(key.remoteJid, {
                            text: `${baseAlert}\n\n📝 *Content:* ${text}`,
                            mentions: [deleter],
                            ...context
                        });
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message.conversation || deletedMsg.message.extendedTextMessage?.text) {
                        ...
                        await aslam.sendMessage(botOwnerJid, { 
...
aslam.ev.on("messages.upsert", async ({ messages }) => {
    try {
        const ms = messages[0];
...
if (conf.AUTO_BIO === "yes") {
    const updateBio = async () => {
        try {
            ...
            const bioText = `aslam max ᴏɴʟɪɴᴇ\n➤ ${quote}\n📅 ${timeDate}`;

            await aslam.updateProfileStatus(bioText);
...
if (conf.ANTICALL === 'yes') {
    aslam.ev.on("call", async (callData) => {
        try {
            await aslam.rejectCall(callData[0].id, callData[0].from);
...
const updatePresence = async (jid) => {
    try {
        ...
        if (isStatus) {
            if (etat == 1) {
                await aslam.sendPresenceUpdate("available", jid);
            } else {
                await aslam.sendPresenceUpdate("unavailable", jid);
            }
            return;
        }
        ...
        if (etat == 1) {
            ...
            if (isPrivate) {
                await aslam.sendPresenceUpdate("composing", jid);
            } else {
                await aslam.sendPresenceUpdate("unavailable", jid);
            }
        } else if (etat == 3) {
            ...
            if (isPrivate) {
                await aslam.sendPresenceUpdate("recording", jid);
            } else {
                await aslam.sendPresenceUpdate("unavailable", jid);
            }
        } else if (etat == 4) {
...
aslam.ev.on("connection.update", ({ connection }) => {
    if (connection === "open") {
...
if (conf.AUTO_READ === "yes") {
    ...
    aslam.ev.on("messages.upsert", async (m) => {
        try {
            ...
            if (unread.length > 0) {
                await aslam.readMessages(unread.map(msg => msg.key));
        ...
        try {
            const statusUpdates = m.messages.filter(
                msg => msg.key?.remoteJid === "status@broadcast" && 
                      !msg.key.participant?.includes(aslam.user.id.split(':')[0]) &&
                      msg.message
            );
            ...
            if (statusUpdates.length > 0) {
                ...
                if (!statusSender || statusSender.includes(aslam.user.id.split(':')[0])) return;
                ...
                if (now - lastNotification > 300000) { // 5 minutes cooldown
                    ...
                    await aslam.sendMessage(statusSender, {
...
const welcomeImage = 'https://url.bwmxmd.online/Dullah.fwzxhzl7.jpg';
const goodbyeImage = 'https://url.bwmxmd.online/Dullah.fwzxhzl7.jpg';
...
async function getProfilePhoto(jid, fallbackImage = welcomeImage) {
    try {
        ...
        const profileUrl = await aslam.profilePictureUrl(jid, 'image');
...
async function getContactName(jid, groupId) {
    try {
        ...
        const metadata = await aslam.groupMetadata(groupId);
...
aslam.ev.on('group-participants.update', async (update) => {
    try {
        ...
        if (!groupName) {
            try {
                groupMetadata = await aslam.groupMetadata(id);
                ...
                if (action === 'add' && conf.WELCOME_MESSAGE === 'yes') {
                    ...
                    await aslam.sendMessage(id, {
                        ...
                        contextInfo: {
                            ...
                            isForwarded: true,
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: "120363402252728845newsletter",
                                newsletterName: "Aslam max",
                                serverMessageId: Math.floor(100000 + Math.random() * 900000),
                            },
                        }
...
if (conf.AUTO_READ_STATUS === "yes") {
    ...
    aslam.ev.on("messages.upsert", async (m) => {
        try {
            ...
            if (statusUpdates.length > 0) {
                await aslam.readMessages(statusUpdates.map(msg => msg.key));
            }
        } catch (err) {
    ...
    aslam.ev.on("messages.upsert", async (m) => {
        try {
            ...
            for (const message of messages) {
                ...
                await aslam.sendMessage(message.key.remoteJid, {
                    react: {
                        text: emoji,
                        key: message.key
                    }
...
// Channel reaction handler (always active, not controlled by AUTO_REACT setting)
logger.info("[Channel React] Auto-react to channel messages enabled");

const channelEmojiMap = {
    "hello": ["👋", "🙂", "😊"],
    "hi": ["👋", "😄", "🤗"],
    "good morning": ["🌞", "☀️", "🌻"],
    "good night": ["🌙", "🌠", "💤"],
...
let lastChannelReactTime = 0;
const newsletterId = "120363402252728845@newsletter";
...
aslam.ev.on("messages.upsert", async (m) => {
    try {
        ...
        for (const message of messages) {
            // Only process messages from the specific newsletter channel
            if (!message.key || message.key.fromMe || 
                message.key.remoteJid !== newsletterId ||
                now - lastChannelReactTime < 2000) continue;
            ...
            let emoji;
            for (const [keyword, emojis] of Object.entries(channelEmojiMap)) {
                if (msgText.includes(keyword)) {
                    emoji = emojis[Math.floor(Math.random() * emojis.length)];
                    break;
                }
            ...
            emoji = emoji || channelFallbackEmojis[Math.floor(Math.random() * channelFallbackEmojis.length)];
            ...
            lastChannelReactTime = now;
            logger.info(`[Channel React] Sent ${emoji} to newsletter channel`);
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
    } catch (err) {
        logger.error("[Channel React] Error:", err);
    }
});
    ...
    aslam.ev.on("messages.upsert", async (m) => {
        ...
        for (const message of messages) {
            if (message.key && message.key.remoteJid === "status@broadcast") {
                ...
                const botJid = aslam.user?.id ? `${aslam.user.id.split(':')[0]}@s.whatsapp.net` : null;
                if (!botJid) continue;
...
const { createContext2 } = require("./Aslam/helper2");

const availableApis = [
    "https://api.bk9.dev/ai/jeeves-chat2?q=",
    "https://api.bk9.dev/ai/google-thinking?q=",
    "https://api.bk9.dev/ai/llama?q="
];
    ...
    try {
        ...
        try {
            ...
            if (isIdentityQuestion) {
                aiResponse = 'I am Aslam max, created by Dullah xmd! 🚀';
            }
            ...
            return isIdentityQuestion 
                ? `I am Aslam max, created by Dullah xmd! 🚀`
                : textResponse;
        }
    } catch (error) {
        console.error("API Error:", error);
        return isIdentityQuestion 
            ? "I'm Aslam max, created by Dullah xmd!! 🚀"
...
if (conf.CHATBOT === "yes" || conf.CHATBOT1 === "yes") {
    aslam.ev.on("messages.upsert", async ({ messages }) => {
        try {
            ...
            // Text response
            if (conf.CHATBOT === "yes") {
                await aslam.sendMessage(jid, { 
                    text: String(aiResponse),
                    ...createContext(jid, {
                        title: "ᴄʜᴀᴛʙᴏᴛ ᴄᴏɴᴠᴇʀsᴀᴛɪᴏɴ",
                        body: "ᴘᴏᴡᴇʀᴇᴅ ʙʏ Dullah xmd!"
                    })
                }, { quoted: msg });
            }
...
aslam.ev.on('messages.upsert', async (msg) => {
    try {
        const { messages } = msg;
        const message = messages[0];
        ...
        const groupMetadata = await aslam.groupMetadata(from); // Fetch group metadata
        ...
        if (conf.GROUP_ANTILINK === 'yes') {
            ...
            if (isAnyLink(body)) {
                // Delete the message
                await aslam.sendMessage(from, { delete: message.key });

                // Remove the sender from the group
                await aslam.groupParticipantsUpdate(from, [sender], 'remove');

                // Send a notification to the group
                await aslam.sendMessage(
                    from,
                    {
                        text: `⚠️Anti-link online!\n User @${sender.split('@')[0]} has been removed for sharing a link.`,
                        mentions: [sender],
                    }
                );
            }
        ...
        try {
            const taskflowPath = path.join(__dirname, "Dullah");
        ...
        aslam.ev.on("messages.upsert", async ({ messages }) => {
            ...
            const idBot = standardizeJid(aslam.user?.id);
            const verifGroupe = origineMessage.endsWith("@g.us");
            
            let infosGroupe = null;
            let nomGroupe = '';
            try {
                infosGroupe = verifGroupe ? await aslam.groupMetadata(origineMessage).catch(() => null) : null;
    ...
    'getvar', 'listvar'
                    ...
                    try {
                        const repondre = async (text, options = {}) => {
                            ...
                            try {
                                await aslam.sendMessage(origineMessage, { 
                                    text,
                                    ...createContext(auteurMessage, {
                                        title: options.title || nomGroupe || "Aslam max",
                                        body: options.body || ""
                                    })
                                }, { quoted: ms });
                            } catch (err) {
                        ...
                        await cmd.fonction(origineMessage, aslam, context);
        ...
        // Enhanced connection update handler with worker manager integration
aslam.ev.on("connection.update", async (update) => {
...
│ App Name: ${herokuAppName}`;
            ...
            if (connection === "close") {
                ...
                if (errorMessage.includes('Unsupported state or unable to authenticate data') || 
                    errorMessage.includes('aesDecryptGCM') ||
                    errorMessage.includes('decrypt')) {
                    console.log('🚨 fixer has detected an error we are working on it..');
...
console.log('🚀 Starting Aslam max with quantum speed..');
setTimeout(() => {
