...
dullah({ nomCom: "vcf", categorie: 'Group', reaction: BOT_BRANDING.EMOJI.CONTACT }, vcfExport);
dullah({ nomCom: "vcard", categorie: 'Group', reaction: BOT_BRANDING.EMOJI.CONTACT }, vcfExport);

// New Group Info Command
dullah({ nomCom: "groupinfo", categorie: 'Group', reaction: "ℹ️" }, async (dest, zk, { repondre }) => {
  try {
    if (!dest.endsWith("@g.us")) {
...
dullah({ nomCom: "backup", categorie: 'Group', reaction: "💾" }, async (dest, zk, { repondre, superUser }) => {
  if (!superUser) {
